...
class DataValidation:
    ...
    def drop_missing_values_columns(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:
        """
        this function will drop columns which contains misssing values more than specify threshold
        df:Accept pandas Dataframe
        threshold: percentage criteria to drop a columns
        =====================================================================================
        return pandas dataframe if atleast single is available after missing coluns drop else none
        """
        try:
            threshold = self.data_validation_config.missing_threshold
            null_report = df.isna().sum()/df.shape[0]
            #selecting column name which has contain null
            logging.info(f"selecting column name which has contain null above to {threshold}")
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            logging.info(f"Drop null values columns train df")
            train_df=self.drop_missing_values_columns(df=train_df,report_key_name="missing_values_within_train_dataset")
            logging.info(f"Drop null values columns test df")
